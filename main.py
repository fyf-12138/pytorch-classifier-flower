...
def parse_opt():
    ...
    parser.add_argument('--model_name', type=str, default='ghostnet', help='model name')
    parser.add_argument('--pretrained', action="store_true", default=True, help='using pretrain weight')
    ...
    parser.add_argument('--amp', action="store_true", default=True, help='using AMP(Automatic Mixed Precision)')
    parser.add_argument('--warmup', action="store_true", default=True, help='using WarmUp LR')
    ...
    parser.add_argument('--metrice', type=str, choices=['loss', 'acc', 'mean_acc'], default='acc',
                        help='best.pt save relu')
    parser.add_argument('--patience', type=int, default=30,
                        help='EarlyStopping patience (--metrice without improvement)')

    # Data Processing parameters
    parser.add_argument('--imagenet_meanstd', action="store_true", default=True, help='using ImageNet Mean and Std')
    ...
    parser.add_argument('--Augment', type=str,
                        choices=['RandAugment', 'AutoAugment', 'TrivialAugmentWide', 'AugMix', 'none'],
                        default='AutoAugment',
                        help='Data Augment')
    ...
    parser.add_argument('--kd_method', type=str, choices=['SoftTarget', 'MGD', 'SP', 'AT'], default='SoftTarget',
    ...
    if opt.resume:
        ...
        print('found checkpoint from {}, model type:{}\n{}'.format(opt.save_path, ckpt['model'].name,
                                                                   dict_to_PrettyTable(ckpt['best_metrice'],
                                                                                       'Best Metrice')))
    else:
...
if __name__ == '__main__':
    ...
    if opt.kd:
        ...
        print('found teacher checkpoint from {}, model type:{}\n{}'.format(opt.teacher_path, teacher_model.name,
                                                                           dict_to_PrettyTable(
                                                                               teacher_ckpt['best_metrice'],
                                                                               'Best Metrice')))
    ...
    for epoch in range(begin_epoch, opt.epoch):
        ...
        begin = time.time()
        if opt.kd:
            model, metrice = fitting_distill(teacher_model, model, loss, kd_loss, optimizer, train_dataset,
                                             test_dataset, CLASS_NUM, DEVICE, scaler,
                                             '{}/{}'.format(epoch + 1, opt.epoch), opt)
        else:
            model, metrice = fitting(model, loss, optimizer, train_dataset, test_dataset, CLASS_NUM, DEVICE, scaler,
                                     '{}/{}'.format(epoch + 1, opt.epoch), opt)
        ...
        if best_metrice is None:
            ...
            if eval('{} {} {}'.format(metrice[0]['test_{}'.format(opt.metrice)], '<' if opt.metrice == 'loss' else '>',
                                      best_metrice['test_{}'.format(opt.metrice)])):
                best_metrice = metrice[0]
                save_model(
